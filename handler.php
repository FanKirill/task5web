...
if ($fullname === '') {
    ...
    $errors['fullname'] = 'Допустимы только буквы, пробелы и дефис';
...
if ($phone !== '') {
    if (!preg_match("/^[\+\d\s\-\(\)]+$/", $phone)) {
        $errors['phone'] = 'Допустимы только цифры, +, пробелы, скобки и дефис';
    } elseif (strlen($phone) > 50) {
        $errors['phone'] = 'Телефон не должен превышать 50 символов';
    }
}
...
if ($birthdate !== '') {
    if (!preg_match("/^\d{4}\-\d{2}\-\d{2}$/", $birthdate)) {
        ...
        if (!$d || $d->format('Y-m-d') !== $birthdate) {
            $errors['birthdate'] = 'Дата не существует';
...
if (!is_array($favLangs) || count($favLangs) === 0) {
    $errors['fav_langs'] = 'Выберите хотя бы один язык';
} elseif (count($favLangs) > 12) {
    $errors['fav_langs'] = 'Выбрано слишком много языков (макс. 12)';
} else {
...
if ($bio !== '' && strlen($bio) > 10000) {
    $errors['bio'] = 'Биография не должна превышать 10000 символов';
}

$contract = $_POST['contract_agreed'] ?? '';
if ($contract !== 'on') {
    $errors['contract_agreed'] = 'Необходимо принять условия соглашения';
}
